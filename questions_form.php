...
class questionnaire_questions_form extends moodleform {
    ...
    function definition() {
        ...
        $quesgroup[] =& $mform->createElement('static', 'qnonedit', '', '<div class="qnonedit">'.
                                              get_string('noneditable', 'questionnaire').'</div>'); // courseeval end
        ...
        foreach ($questionnaire->questions as $question) {
            ...
            /// Need to index by 'id' since IE doesn't return assigned 'values' for image inputs.
				// courseeval start
				if ($questionnaire->qtype !== QUESTIONNAIRECOURSEEVAL || $question->noneditable == 'n' || is_siteadmin($USER->id)) {
	                ...
	                ${$quesgroup}[] =& $mform->createElement('image', 'removebutton['.$question->id.']', $rsrc, $rextra);
	    		 	...
	    		 	${$quesgroup}[] =& $mform->createElement('static', 'opentag_'.$question->id, '', '<div class="qicons">');
					${$quesgroup}[] =& $mform->createElement('static', 'spaces', '', "&nbsp");
					${$quesgroup}[] =& $mform->createElement('static', 'spaces', '', "&nbsp");
					${$quesgroup}[] =& $mform->createElement('static', 'spaces', '', "&nbsp");
					${$quesgroup}[] =& $mform->createElement('static', 'spaces', '', "&nbsp");
					${$quesgroup}[] =& $mform->createElement('static', 'spaces', '', "&nbsp");
            ...
            ${$quesgroup}[] =& $mform->createElement('static', 'qnoneditable_'.$question->id, '', '<div class="qnonedit">'.$qnoneditable.'</div>'); // courseeval end
    ...
    function definition() {
        ...
        if ($question->type_id != QUESSECTIONTEXT && $question->type_id != '') {
			...
			if (is_siteadmin($USER->id)) {
	            ...
	            $mform->setHelpButton('noneditgroup', array('noneditable', get_string('noneditable', 'questionnaire'), 'questionnaire'));
			} // courseeval end
        }
